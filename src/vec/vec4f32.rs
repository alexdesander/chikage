...
impl IndexMut<usize> for Vec4f32 {
...
impl Mul<Vec4f32> for f32 {
    ...
    fn mul(self, mut v: Vec4f32) -> Self::Output {
        v[0] *= self;
        v[1] *= self;
        v[2] *= self;
        v[3] *= self;
...
mod tests {
    ...
    fn mag_and_mag_squared() {
        let zero = Vec4f32::zero();
        let ones = Vec4f32::ones();
        let v = Vec4f32::new([4.0, 7.0, 5.0, 2.0]);

        assert_eq!(zero.mag(), 0.0);
        assert_eq!(ones.mag(), 4.0f32.sqrt());
        assert_eq!(
            v.mag(),
            (4.0 * 4.0 + 7.0 * 7.0 + 5.0 * 5.0 + 2.0 * 2.0f32).sqrt()
        );

        assert_eq!(zero.mag_squared(), 0.0);
        assert_eq!(ones.mag_squared(), 4.0f32);
        assert_eq!(
            v.mag_squared(),
            4.0 * 4.0 + 7.0 * 7.0f32 + 5.0 * 5.0 + 2.0 * 2.0f32
        );
    }
    ...
    fn vector_addition() {
        let mut v = Vec4f32::new([4.0, 7.0, 5.0, 2.0]);
        let w = Vec4f32::new([-2.0, 10.0, 2.0, 3.0]);
        let r = v + w;
        assert_eq!(r[0], 2.0);
        assert_eq!(r[1], 17.0);
        assert_eq!(r[2], 7.0);
